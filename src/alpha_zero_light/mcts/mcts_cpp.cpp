...
public:
    ...
    GomokuGame(int size = 9, int length = 5) : board_size(size), win_length(length) {}
...
public:
    ...
    Node(const std::vector<float>& s, Node* p, int action, float prior_prob, int player)
    ...
    float get_ucb(const Node* child, float c_puct) const {
        ...
        float q_value = 1.0f - ((child->value_sum / child->visit_count) + 1.0f) / 2.0f;
        float u = c_puct * child->prior * std::sqrt((float)visit_count) / (1.0f + child->visit_count);
        return q_value + u;
    }
    ...
    void expand(const std::vector<float>& policy, const std::vector<int>& valid_moves, int board_size) {
        ...
        for (int action : valid_moves) {
            ...
            if (prob > 0) {
                ...
                // AlphaZero usually flips perspective so the neural net always sees "me" as 1.
                // If we do that, we need to flip the board values.
                // Let's stick to the logic:
                // Current node state is from perspective of player_to_move.
                // Child node state should be from perspective of -player_to_move.
                ...
                // The move we just made (player_to_move) becomes -1 in the new state (opponent)
                // Wait, if we set it to player_to_move (1) then flip, it becomes -1. Correct.
                
                children.push_back(std::make_unique<Node>(
                    next_state, this, action, prob, -player_to_move
                ));
            }
        }
    }
    ...
    void backpropagate(float value) {
        ...
        if (parent != nullptr) {
            // Value is from perspective of player who just moved (parent's player_to_move)
            // So for parent, it is the value.
            // But wait, standard MCTS backprop flips value.
            // If child value is V (for child's player), then for parent it is -V.
...
public:
    ...
    // model_func: python function that takes batch of states and returns (policies, values)
    std::vector<std::vector<float>> search_batch(
        const std::vector<py::array_t<float>>& initial_states,
        ...
        for (const auto& state_np : initial_states) {
            ...
            for (int i = 0; i < r.size(); ++i) state_vec[i] = r(i);
        ...
        for (int search = 0; search < num_searches; ++search) {
            ...
            for (int i = 0; i < batch_size; ++i) {
                ...
                // Check terminal
                // Current state is from perspective of player_to_move
                // But check_win checks if the *previous* player won.
                // In our state rep, previous player is -1 (opponent).
                // But check_win implementation expects 1 or -1.
                // Let's look at the board.
                // If I just moved, I am -1 in the current state (because of flip).
                // So we check if -1 has won.
                ...
                if (game.check_win(node->state.data(), -1)) {
                    terminated = true;
                    value = -1.0f; // Loss for current player
            ...
            if (leaf_nodes.empty()) continue;
            
            // 2. Prepare batch for NN
            // We need to convert leaf states to the format expected by the model
            // Model expects (B, 3, H, W) usually.
            // Our state is flat (H*W).
            // We need to construct the 3 planes: [Opponent, Empty, Self]
            // Note: Our state is [Self=1, Opponent=-1, Empty=0]
            // So:
            // Plane 0 (Opponent): state == -1
            // Plane 1 (Empty): state == 0
            // Plane 2 (Self): state == 1
            
            int num_leaves = leaf_nodes.size();
            int area = board_size * board_size;
            
            // Create a numpy array for the batch: (B, 3, H, W)
            // We'll create a flat float array and reshape in Python or here
            ...
            for (int i = 0; i < num_leaves; ++i) {
                ...
                for (int j = 0; j < area; ++j) {
                    ...
                    batch_data[i*3*area + 0*area + j] = (val == -1.0f) ? 1.0f : 0.0f;
                    // Channel 1: Empty (0)
                    batch_data[i*3*area + 1*area + j] = (val == 0.0f) ? 1.0f : 0.0f;
                    // Channel 2: Self (1)
                    batch_data[i*3*area + 2*area + j] = (val == 1.0f) ? 1.0f : 0.0f;
                }
            }
            
            // Call Python model
            py::array_t<float> input_np({num_leaves, 3, board_size, board_size}, batch_data.data());
            
            // Returns tuple (policies, values)
            // policies: (B, 81), values: (B, 1)
            ...
            auto values_r = values_np.unchecked<2>(); // (B, 1) or (B,)
            
            // 3. Expansion and Backprop
            for (int i = 0; i < num_leaves; ++i) {
                Node* node = leaf_nodes[i];
                float value = values_r(i, 0); // Value for current player
                
                // Get valid moves
                std::vector<int> valid_moves = game.get_valid_moves(node->state.data());
                
                // Extract policy and mask
